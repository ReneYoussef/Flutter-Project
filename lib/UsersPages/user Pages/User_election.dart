...
class User_election extends StatefulWidget {
  const User_election({Key? key});

  @override
  State<User_election> createState() => _User_electionState();
}

class _User_electionState extends State<User_election> {
...
// get the number of coucil members in the user City according to uservillage  name
                    ...
                    Expanded(
                      ...
                      stream: FirebaseFirestore.instance
                          ...
                          .where('Candidate Village',
                              isEqualTo: currentUserDisplayCity)
                          .snapshots(),
                      builder:
                        ...
                        return Column(children: [
                          Row(
                            children: [
                              ElevatedButton(
                                onPressed: () async {
                                  CollectionReference votesRef = FirebaseFirestore
                                      .instance
                                      .collection('votes');
                                  CollectionReference listVotesRef =
                                      FirebaseFirestore.instance
                                          .collection('votes_lists');
                                  CollectionReference cityRef = FirebaseFirestore
                                      .instance
                                      .collection('Cities');
                                  ...
                                  if (selectedCandidates.isNotEmpty) {
                                    String villageName =
                                        selectedCandidates[0]['village'];
                                    QuerySnapshot citySnapshot = await cityRef
                                        .where('name', isEqualTo: villageName)
                                        .get();

                                    if (citySnapshot.docs.isNotEmpty) {
                                      DocumentSnapshot cityDoc =
                                          citySnapshot.docs.first;
                                      int maxCouncilMembers =
                                          cityDoc['Council members'];
                                      QuerySnapshot userVoteSnapshot =
                                          await votesRef
                                              .where('userId', isEqualTo: userId)
                                              .where('City', isEqualTo: villageName)
                                              .get();

                                      if (userVoteSnapshot.docs.length >=
                                          maxCouncilMembers) {
                                        _showDialog(
                                          scaffoldContext,
                                          'Vote Error',
                                  ...
                                  for (Map<String, dynamic> data
                                      in selectedCandidates) {
                                    QuerySnapshot candidateVotesSnapshot =
                                        await votesRef
                                            .where('userId', isEqualTo: userId)
                                            .where('candidateId',
                                                isEqualTo: data['candidateId'])
                                            .get();
                                  ...
                                  showDialog(
                                    ...
                                    builder: (BuildContext context) {
                                      return AlertDialog(
                                        ...
                                        content: Text(
                                            'You have selected: ${selectedCandidates.map((data) => data['name']).join(', ')}\n\nAre you sure you want to vote for them?'),
                                        actions: [
                                          TextButton(
                                            onPressed: () {
                                          ...
                                          TextButton(
                                            onPressed: () async {
                                              ...
                                              for (Map<String, dynamic> data
                                                  in selectedCandidates) {
                                                bool canVote =
                                                    await _checkElectionStatus(
                                                        data['village']);
                                                if (canVote) {
                                                  await _voteForIndividualCandidate(
                                                      scaffoldContext,
                                                      data['candidateId'],
                                                      data['name'],
                                                      data['village']);
                              ...
                              Row(
                                children: [
                                  ValueListenableBuilder<int>(
                                    valueListenable:
                                        selectedCandidatesCountNotifier,
                                  ...
                                  Icon(Icons.groups_sharp,
                                      color: Color(0xFA1100FF).withOpacity(0.75)),
                                ],
                              ),
                            ],
                          ),
                          Expanded(
                            child: ListView.builder(
                              ...
                              itemBuilder: (BuildContext context, int index) {
                                var userData = candidateDocs[index].data()
                                    as Map<String, dynamic>;
                                String imageUrl = userData['imagelink'] ??
                                    ''; // Fetch the image URL from user data
                                return Column(
                                  children: [
                                    Container(
